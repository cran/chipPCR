...
function(x, y, trans = TRUE, bg.outliers = FALSE, median = FALSE, minmax = FALSE, qnL = 0.1, 
  ...
  if (trans) {
    if (class(try(lmrob(y[c(BG)] ~ x[c(BG)]), silent = TRUE)) == "try-error") { 
    ...
    if (bg.outliers) { 
      y.norm[c(BG)] 	<- rm.outlier(y.norm[c(BG)], fill = TRUE, median = median)
      y.norm[c(BG)] 	<- rm.outlier(y.norm[c(BG)], opposite = TRUE, fill = TRUE, 
				      median = median)
    } 
